...
**Date**: July 27, 2025, 13:30  
**Component**: Explorer File Operations Service  
**Status**: Technical Documentation  
**Priority**: High

## Overview

This document provides a detailed implementation of the FileOperationsService, which handles core file operations (copy, paste, move, rename, delete, duplicate) in the Explorer component. The service ensures consistent behavior across the application and integrates with the undo/redo system.
...
from PySide6.QtCore import QObject, Signal, Slot, QMimeData
...
class FileOperationsService(QObject):
    ...
    def copy_to_clipboard(self, paths: List[str]) -> bool:
        ...
        urls = [f"file://{path}" for path in valid_paths]
    ...
    def get_clipboard_contents(self) -> Tuple[str, List[str]]:
        """
...
file_ops.operationFailed.connect(lambda op_type, sources, error: 
                                print(f"Operation {op_type} failed: {error}"))
...
```python
def create_context_menu(selected_paths, parent_widget):
    ...
    if selected_paths:
        ...
        copy_action.triggered.connect(lambda: file_ops.copy_to_clipboard(selected_paths))
        
        cut_action = menu.addAction("Cut")
        cut_action.triggered.connect(lambda: file_ops.cut_to_clipboard(selected_paths))
        
        delete_action = menu.addAction("Delete")
        delete_action.triggered.connect(lambda: file_ops.delete_items(selected_paths))
        
        if len(selected_paths) == 1:
            rename_action = menu.addAction("Rename")
            rename_action.triggered.connect(lambda: show_rename_dialog(selected_paths[0]))
            
            duplicate_action = menu.addAction("Duplicate")
            duplicate_action.triggered.connect(lambda: file_ops.duplicate_item(selected_paths[0]))
    
    # Add paste action if clipboard has content
    clipboard_mode, clipboard_paths = file_ops.get_clipboard_contents()
    if clipboard_mode and is_directory(parent_path):
        paste_action = menu.addAction("Paste")
        paste_action.triggered.connect(lambda: file_ops.paste(parent_path))
    ...
    if is_directory(parent_path):
        ...
        new_file_action.triggered.connect(lambda: file_ops.create_new_file(parent_path))
        
        new_folder_action = new_menu.addAction("Folder")
        new_folder_action.triggered.connect(lambda: file_ops.create_new_folder(parent_path))
...
def dropEvent(self, event):
    ...
    if event.mimeData().hasUrls():
        ...
        drop_path = self.model().filePath(drop_index) if drop_index.isValid() else self.rootPath()
        
        # Get source paths from mime data
        source_paths = [url.toLocalFile() for url in event.mimeData().urls()]
        
        # Handle drop operation
        if event.keyboardModifiers() & Qt.ShiftModifier:
            # Cut/move operation
            self.file_operations.move_items(source_paths, drop_path)
        else:
            # Copy operation
            self.file_operations.copy_to_clipboard(source_paths)
            self.file_operations.paste(drop_path)
...
The service handles errors comprehensively:

1. **Pre-operation validation**: Checks for common issues (permissions, existence) before attempting operations
2. **Operation sandboxing**: Each operation runs in a try/except block with proper error reporting
3. **Signal-based notifications**: Emits signals for operation start, completion, and failure
4. **Logging**: Detailed error logging with context information
5. **Rollback capability**: Through the undo system, provides recovery from failed operations

## Undo/Redo System

The undo/redo system is designed to work with complex file operations:

1. **Comprehensive metadata**: Each operation stores enough metadata to fully reverse it
2. **Content preservation**: For small files, content is stored to enable restoration after deletion
3. **Chain awareness**: Operations are recorded in sequence for proper undo/redo chain
4. **Error recovery**: If an undo/redo fails, the operation remains in the stack for retry
5. **Selective undoable operations**: Permanent operations (like delete with skip_trash) are marked as non-undoable

## Performance Considerations

The service is designed with performance in mind:

1. **Progressive execution**: For bulk operations, files are processed one by one
2. **Size limits**: Content preservation for undo has size limits to prevent memory issues
3. **Progress tracking**: Operations are tracked with UUIDs for potential progress reporting
4. **Clipboard optimization**: Internal clipboard is synchronized with system clipboard
5. **Path validation**: Paths are validated early to avoid wasted processing
